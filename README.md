...
- ⏰ 定时自动签到：**本机**（Windows 计划任务）六档 21:35 / 21:50 / 22:05 / 22:20 / 22:28 / 23:40 准点可靠（22:20 / 22:28 两档专防「晚开机」）；**云端**（GitHub Actions）16:30–23:55 每 30 分钟一档、共 16 档，按 GitHub 延迟特性设计，几小时的延迟照样有档落窗
...
在意准点、或者完全不想碰 GitHub：用**电脑浏览器**打开[配置页](https://cccccheyu.github.io/fzu-auto-checkin/)填完 1~3 步（手机上生成的话要把 bat 传回电脑再双击），在「方式 B」里下载 `setup.bat`，**双击一次全装好**——脚本会自动检查 Python（没装会给出国内镜像地址）、下载项目代码、写入配置、装依赖、创建每天 21:35 / 21:50 / 22:05 / 22:20 / 22:28 / 23:40 的计划任务，最后试运行一次（只验证，不真签）。
...
4. 云端 + 本地一起开最稳：两边逻辑幂等——先到的签掉，后到的识别「已签到」自动静默跳过
   （云端那侧不会重新登录、不会改你的 token，可放心和本机同时开）
...
**22:30**。21:30–22:30 打卡为正常，22:30–23:59 打卡记迟到，**23:59 服务器关窗**。这三个都是服务器自己给的（`init` 返回的 `paramsData`：`FStartTime 21:30` / `FLateTime 22:30` / `FEndTime 23:59`）。本机六档 21:35 / 21:50 / 22:05 / 22:20 / 22:28 都在迟到线之前（后两档专防「关机后晚开机」——计划任务错过档位后会在开机时补跑，22:20 / 22:28 保证补跑仍落在正常区），23:40 保底宁迟不缺；云端另有 16:30–23:55 十六档，按延迟特性铺开。
...
用**电脑浏览器**打开[配置页](https://cccccheyu.github.io/fzu-auto-checkin/)，填完 1~3 步后在「方式 B」下载 `setup.bat`，双击一次全装好（自动查 Python、下载代码、写配置、装依赖、建每天 21:35 / 21:50 / 22:05 / 22:20 / 22:28 / 23:40 六个计划任务，最后试运行一次）。
