...
def login(username: str, password: str) -> str:
    ...
    s = requests.Session()
    # 强制直连：登录只面向校方 SSO，绝不能走系统代理（Clash 曾把出口变成境外 IP，
    # 且节点全挂时会直接导致登录失败 → 整晚签不上）。见 src/checkin.py 同样的处理。
    s.trust_env = False
    ...
    # 失败：尝试从返回页面提取错误信息。
    # ⚠️ 页面里这段提示是 <div id="login-error-msg"><span>认证信息无效。</span></div>，
    # 文案裹在内层 <span> 里。以前用 `>[^<]+` 取，只会拿到标签之间的换行空白，
    # strip 之后变成空字符串 → 日志与推送里只剩「SSO 登录失败: 」，
    # 2026-09-21/22 排查改密问题时正是被这个空消息拖慢了。现在先整块取出再剥标签。
    m = re.search(r'id="login-error-msg"[^>]*>(.*?)</div>', r2.text, re.S)
    hint = re.sub(r"<[^>]+>", " ", m.group(1)) if m else ""
    hint = re.sub(r"\s+", " ", hint).strip()
    if not hint:
        hint = "未知错误（页面未给出提示，可能需要验证码或页面结构已变化）"
